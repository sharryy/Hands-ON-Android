...
public class SaveCardToFileWorker extends Worker {
    ...
    public static final SimpleDateFormat DATE_FORMAT = new SimpleDateFormat("yyyy.MM.dd 'at' HH:mm:ss z", Locale.getDefault());
    ...
    public Result doWork() {
        ...
        try {
            ...
            String outputUri = MediaStore.Images.Media.insertImage(
                    resolver, bitmap, TITLE, DATE_FORMAT.format(new Date())
            );

            if (TextUtils.isEmpty(outputUri)) {
                Log.i(TAG, "Writing to Media Failed: ");
                return Result.failure();
            }

            Data outputData = new Data.Builder()
                    .putString(KEY_IMAGE_URI, outputUri).build();

            return Result.success(outputData);
        } catch (Exception e) {
            Log.i(TAG, "Unable to save Images to  Gallery", e);
